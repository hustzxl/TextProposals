...
static double cal_angle(cv::Point p1,cv::Point p2){
    double angle = 0;
...
static void cal_angle_diff(cv::Mat img,const std::vector<int>& index,std::vector<Region>&region,float& mean,float& stdev){
    int len = index.size();
    std::vector < double> angle;
    angle.clear();
    for (int i = 0;i < len;i++){
        Region re = region[ index[i] ];
        cv::Point p1 (re.bbox_.x + re.bbox_.width/2,re.bbox_.y + re.bbox_.height/2);
        rectangle(img,re.bbox_,(0,0,255),2);
        circle(img,p1,1,(0,255,0));
        for (int j = 0;j < len ; j++){
            if (i == j){
                continue;
                angle.push_back(0);
            }
            Region re2 = region[ index[j] ];
            cv::Point p2 (re2.bbox_.x + re2.bbox_.width/2,re2.bbox_.y + re2.bbox_.height/2);
            angle.push_back(cal_angle(p1,p2));
            rectangle(img,re2.bbox_,(0,255,0),2);
            circle(img,p2,1,(0,255,0));
        }
    }
...
*/
  ...
  for (t_index j=1; j< N- 1; j++) {
    ...
    float mean =0,stdev = 0,radio = 0;
    //cal_angle_diff(img,index,region,mean,stdev);
    ...
    //std::cout<<"prev node : "<<prev_node<<" follow node : "<<idx2<<std::endl;
  }
}
...
static void generate_dendrogram(t_float * const Z, cluster_result & Z2, const t_index N) {
  ...
  for (t_index i=0; i<N-1; i++) {
    ...
    //std::cout<<"size_ node1: "<<size_(node1)<<" size_ node2: "<<size_(node2)<<std::endl;
...
static int linkage_vector(t_float *X, int N, int dim, t_float * Z, unsigned char method, unsigned char metric,std::vector<Region>&regions,cv::Mat& img) {
  ...
  try{
    ...
    case METHOD_METR_SINGLE:
      ...
      MST_linkage_core_vector(N, dist, Z2,regions,img);
      break;
    case METHOD_METR_WARD:
