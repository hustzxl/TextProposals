...
public:
	...
	/// Constructor.
	HierarchicalClustering(vector<Region> &regions,Mat& img);
    ...
    void info_merge(HCluster& cluster,vector<float>&sample);	

private:
    vector<Region> &regions;
    CvBoost boost;
    Mat img;
    double cal_angle(Point,Point);
    int cal_angle_diff(Mat,vector<int>&,float&,float&);
};
